...
@section('title', __('public.title.title permission create'))
@section('vendor-css')
...
@section('content')
    ...
    <div class="card mb-4">
        <h5 class="card-header" style="text-align: center">@lang('public.title.title permission create')</h5>
        <form class="card-body" action="{{ route('permissions.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" id="multicol-first-name"
                        class="form-control" placeholder="عنوان دسترسی را به انگلیسی وارد کنید">
                    <span class="error">
                        @error('name')
                            {{ $message }}
                        @enderror
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="persian_name" value="{{ old('persian_name ') }}" id="multicol-last-name"
                        class="form-control" placeholder="عنوان دسترسی را به  فارسی وارد کنید.">
                    <span class="error">
                        @error('persian_name')
                            {{ $message }}
                        @enderror
